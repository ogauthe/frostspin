...
def test_ctmrg(tmp_path):
    # Consider random tensors with each bond having a different representation of
    # different size. SequentialCTMRG will crash if any mismatch appears in leg
    # contractions.
...
def test_unit_cell(tmp_path):
    """
    Exhaustive test for SequentialCTMRG unit cell. Construct a 4x4 unit cell with
    all 32 inequivalent bond differ from each other. Use U(1): each bond has
    dimension 4, but has different quantum numbers and contraction with any other
    bond will fail. Also consider charge conjugation: conjugate representation does
    not appear in any other bond.
