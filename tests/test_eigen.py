...
def test_eigen():
    ...
    vsu2 = mat_su2.truncated_eig(
        nvals, dmax_full=dmax_full, rng=rng, compute_vectors=False
    )
    assert vsu2.dtype == np.complex128

    vsu2 = vsu2.toarray(sort=True)[:nvals]
    vo2 = mat_o2.truncated_eig(
        nvals, dmax_full=dmax_full, rng=rng, compute_vectors=False
    )
    vo2 = vo2.toarray(sort=True)[:nvals]
    vu1 = mat_u1.truncated_eig(
        nvals, dmax_full=dmax_full, rng=rng, compute_vectors=False
    )
    vu1 = vu1.toarray(sort=True)[:nvals]
    vas = mat_as.truncated_eig(
        nvals, dmax_full=dmax_full, rng=rng, compute_vectors=False
    )
    vas = vas.toarray()[:nvals]

    # due to degen, actual sizes may be larger than nvals
    # values are sorted by absolute value, meaning pairs of complex conjugate may
    # be swapped. Check that values match either the reference or its conjugate.
    d1 = default[:nvals]
    d2 = d1.conj()

    # This test is a bit brittle in the asymmetric case: ghost elimination in the
    # presence of exact degeneracies is tricky. This is a limitation of Krylov methods
    # here we just choose that seed that works.
    ...
    mat_missing = SU2SymmetricTensor(
        reps, reps, blocks, block_irreps, mat_su2.signature
    )
    ...
    assert all(
        lg.norm(check[i] - np.diag(d)) < 1e-12
        for i, d in enumerate(vals.diagonal_blocks)
    )
