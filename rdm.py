...
def rdm_1x1(C1,T1,C2,T4,A,T2,C4,T3,C3):
  ...
  rdm = np.dot(T4.reshape(T4.shape[0]*T4.shape[1], T4.shape[2]),C4).reshape(T4.shape[0]*T4.shape[1], C4.shape[1])
  ...
  T2C2 = np.dot(T2C2,C2).reshape(T2.shape[1]*T2.shape[2], C2.shape[1])
  ...
  del T2C2, T1C1
  T2C2T1C1 = T2C2T1C1.transpose(2, 4, 3, 1, 0).reshape(A.shape[2]*A.shape[1], C1.shape[1]*A.shape[1]*A.shape[2]*T2.shape[1])
  rdm = rdm.transpose(2, 1, 4, 3, 0).reshape(T2C2T1C1.shape[1], T4.shape[0]*A.shape[0])
