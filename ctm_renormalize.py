...
def construct_projectors(R,Rt,chi,verbosity=0):
  assert(D2*last_chi == R.shape[0] == Rt.shape[0])
  if verbosity > 1:
...
def renormalize_C1_up(env,x,y,verbosity=0):
  ...
  #  |
  #  2 -> 1
  nC1 = nC1.reshape(len(P),T4.shape[1])
  #  C1-\
  #  |   00=P-1 ->0
  #  T4-/
  #  |
...
def renormalize_T1(env,x,y,verbosity=0):
  ...
  D2 = a.shape[2]
  chi = Pt.shape[1]
  P_ = P.reshape(last_chi,D2,chi).tranpose(1, 2, 0).reshape(D2*chi, last_chi)
  nT1 = np.dot(P_,T1.reshape(last_chi, D2*last_chi)).reshape(D2, chi, D2, last_chi)
  del P_
  nT1 = nT1.tranpose(1, 3, 2, 0).reshape(chi*last_chi, D2**2)
  nT1 = np.dot(nT1,a.reshape(D2**2, D2**2)).reshape(chi, last_chi, D2, D2)
  nT1 = nT1.tranpose(0, 2, 1, 3).reshape(D2*chi, Pt.shape[0])
...
def renormalize_T2(env,x,y,verbosity=0):
  ...
  D2 = a.shape[0]
  chi = P.shape[1]
  Pt_ = Pt.reshape(last_chi,D2,chi),tranpose(1, 2, 0).reshape(D2*chi, last_chi)
  nT2 = np.dot(Pt_,T2.reshape(last_chi, D2*last_chi)).reshape(D2, chi*last_chi, D2)
  del Pt_
  nT2 = nT2.tranpose(1, 0, 3).reshape(chi*last_chi, D2**2)
...
def renormalize_T3(env,x,y,verbosity=0):
  ...
  nT3 = nT3.transpose(0,2,1,3).reshape(chi*last_chi,D2**2)
...
def renormalize_T4(env,x,y,verbosity=0):
  ...
  nT4 = nT4.tranpose(3,0,2,1).reshape(chi*last_chi, D2**2)
  nT4 = np.dot(nT4,a.reshape(D2**2, D2**2)).reshape(chi, P.shape[0], D2)
  nT4 = nT4.transpose(1,2,0).reshape(D2*chi, P.shape[0])
  nT4 = np.dot(P,nT4).reshape(chi, D2, chi)
  nT4 = nT4/lg.norm(nT4)
  return nT4
